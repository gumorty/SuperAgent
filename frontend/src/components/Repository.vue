...
const resolveAgentName = (item) => item?.name || item?.agentName || item?.templateName || '未命名 SuperAgent';
...
const sections = computed(() => [
    {
        ...
        emptyText: '暂无已创建的 SuperAgent'
    },
    {
        key: 'added',
        title: '我添加的',
        items: addedAgents.value,
        emptyText: '暂无已添加的 SuperAgent'
    },
    {
        key: 'favor',
        title: '我收藏的',
        items: favorAgents.value,
        emptyText: '暂无已收藏的 SuperAgent'
    }
]);
...
const deleteMineAgent = async (item) => {
    if (!item?.agentId) {
        message.value = '该条目缺少可删除的 SuperAgent 标识';
        return;
    }
    ...
    try {
        ...
        notifyAppError(error, '删除 SuperAgent 失败');
        loading.value = false;
    }
};
        ...
        <div class="overflow-y-auto py-[24px] pl-[24px] pr-[calc(24px+var(--scrollbar-w))]">
            <div class="mx-auto max-w-[1100px] space-y-[16px]">
                <header class="flex items-center justify-between gap-[12px]">
                    <h1 class="text-[24px] font-bold text-[var(--text-primary)]">SuperAgent Repository</h1>
                </header>
