...
const fetchAgents = async () => {
    try {
        ...
        console.warn('获取 SuperAgent 列表失败', error);
        return [];
    }
};
...
const consumeWelcomeLaunchTask = async () => {
    ...
    if (!currentAgentId.value) {
        sendError.value = '暂无可用 SuperAgent，请先在后台配置 SuperAgent';
        return;
    }
            ...
            >
                <div class="flex items-center gap-[10px]">
                    <div class="flex items-center gap-[14px] font-semibold">
                        <label class="w-[72px] text-[14px] text-[var(--text-secondary)] text-right">SuperAgent</label>
