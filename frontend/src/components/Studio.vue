...
const doGenerate = async () => {
    if (!form.agentName.trim()) {
        message.value = '请输入 SuperAgent 名称';
        return;
    }
        ...
        <div class="overflow-y-auto py-[24px] pl-[24px] pr-[calc(24px+var(--scrollbar-w))]">
            <div class="mx-auto max-w-[1100px] space-y-[16px]">
                <div class="flex items-center justify-between gap-[12px]">
                    <h1 class="text-[24px] font-bold text-[var(--text-primary)]">SuperAgent Studio</h1>
                </div>

                <div class="space-y-[18px] rounded-[16px] bg-white p-[18px]">
                    <div class="flex min-h-[50px] items-center gap-[12px]">
                        <div class="flex h-[44px] w-[140px] shrink-0 items-center text-[16px] font-semibold tracking-[0.02em] text-[var(--text-secondary)]">SuperAgent 名称</div>
                    ...
                    <div class="space-y-[12px]">
                        <textarea
                            ...
                            placeholder="描述你希望 SuperAgent 完成的任务目标、输入上下文、执行约束和产出格式。例如：每周一早上 9 点汇总上周投放数据，给出异常原因与优化建议，并输出可直接发送给团队的简报。"
                        ></textarea>
